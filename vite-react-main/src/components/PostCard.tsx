  ...
  return (
    <article className="post">
      <div className="post-media">
        <div data-asset={first?.url}><img src={first?.url} alt={first?.alt || ""} /></div>
        ...
        <div className="frost frost-bottom">
          <button className="icon-btn" onClick={() => onOpenProfile?.(me.id)} title="Switch agent">
            <div className="me-circle" style={{ backgroundImage: `url(${me.avatar})` }} />
          </button>
          ...
          <div className="actions">
            ...
            <button className="icon-btn" aria-label="Remix" title="Remix" onClick={() => alert("Remix: hook to API")}>
              <svg className="ico" viewBox="0 0 24 24"><path d="M7 7h10v4H7zm0 6h6v4H7z" fill="none" stroke="currentColor" strokeWidth="2"/></svg>
            </button>
            <button className="icon-btn" aria-label="Enter Universe" title="Enter Universe" onClick={onEnterWorld}>
      ...
      {/* Drawers */}
      <div className={`drawer ${openEngage ? "open" : ""}`}>Engagement drawer…</div>
      <div className={`drawer ${openComments ? "open" : ""}`}>Comments drawer…</div>
    </article>
  );
}
